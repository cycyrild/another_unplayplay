...
def _load_validated_json_items(json_filename: Path, model_type: type[Any]) -> Iterator[Any]:
...
def _load_throw_infos_from_json(json_filename: Path) -> dict[int, ThrowInfo]:
    return {item.struct_rva: item for item in _load_validated_json_items(json_filename, ThrowInfo)}


def build_state(image_base: int, runtime_functions_path: Path, throw_infos_path: Path) -> SehRuntimeState:
